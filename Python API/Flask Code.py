...
def home():
	...
	def select_image():
		...
		#urlis="https://firebasestorage.googleapis.com/v0/b/myecommerceshoppingapp.appspot.com/o/note%20image.jpg?alt=media&token=069c925b-1177-4fb0-81a3-b702a5b33bf9"
		urlis =str((request.args['img']) +"&token="+ (request.args['token']))
		urlis = urlis[:80] + "%2F" + urlis[81:]
		#path = "/home/godseye/mysite/Dataset/500_dataset/500_s1.jpg"
		selectedImage = True
		...
		def testFeature_1_2_7():
			...
			for j in range(NUM_OF_FEATURES):
				...
				for i in range(NUMBER_OF_TEMPLATES):
					...
					# Current template
					cur_dir = os.getcwd()
					template_path = '/home/godseye/mysite/Dataset/500_Features_Dataset/Feature ' + str(j+1) + '/' + str(i+1) + '.jpg'
	...
	def testing_100():
		...
		search_area_list = [[180,300,195,370],
                            [1025,1470,290,468],
                            [100,450,20,120],
                            [690,1050,20,120],
                            [820,1030,320,430],
		...
		def testFeature_1_2_7():
			...
			for j in range(NUM_OF_FEATURES):
				...
				for i in range(NUMBER_OF_TEMPLATES):
					...
					# Current template
					cur_dir = os.getcwd()
					template_path = '/home/godseye/mysite/Dataset/100_Features_Dataset/Feature ' + str(j+1) + '/' + str(i+1) + '.jpg'
	...
	def testing_200():
		...
		def testFeature_1_2_7():
			...
			for j in range(NUM_OF_FEATURES):
				...
				for i in range(NUMBER_OF_TEMPLATES):
					...
					# Current template
					cur_dir = os.getcwd()
					template_path = '/home/godseye/mysite/Dataset/200_Features_Dataset/Feature ' + str(j+1) + '/' + str(i+1) + '.jpg'
		...
		def testResult():
			...
			# The feature passes the test if number of bleed lines is close to 7 (6.7 - 7.6)
			if line_count >= 4.7 and line_count <= 5.9:
				status = True
				successful_features_count += 1
				print('Feature 8: Successful- 5 bleed lines found in left part of currency note')
				return_list[7] = {
	...
	if selectedImage == True:
		if option == 3:                        # For 500 currency note
			return jsonify(testing_500())
		elif option == 1:                      # For 100 currency note
			return jsonify(testing_100())
		elif option == 2:                      # For 100 currency note
			return jsonify(testing_200())
